...
SOURCES = {"FRAG": "index__FRAG.frag.glsl", "LFRAG": "index__LFRAG.frag.glsl", "VERT": "index__VERT.vert.glsl",
           ...
           # the ambient page (2026-09-08): its globe and its cloud shell, cross-checked against ambient.html
           "AFRAG": "ambient__AFRAG.frag.glsl", "AVERT": "ambient__AVERT.vert.glsl",
           "ACFRAG": "ambient__ACFRAG.frag.glsl", "ACVERT": "ambient__ACVERT.vert.glsl"}
# the JS source that carries each shader's material (app.js unless named here)
JS_FOR = {"AFRAG": "ambient.html", "ACFRAG": "ambient.html"}
...
def main():
    ...
    app_js = open(APP).read() if os.path.exists(APP) else ""
    for name, body in shader_blocks():
        if not name.endswith("FRAG"):
            continue
        code = _strip_comments(body)
        js_name = JS_FOR.get(name, "app.js")
        app = open(os.path.join(WEB, js_name)).read() if js_name != "app.js" else app_js
        found = 0
        # the material names the shader bare (const CFRAG=SHADERS.CFRAG.replace(...)) or as SHADERS.NAME
        for m in re.finditer(r"fragmentShader:\s*(?:SHADERS\.)?" + name + r"\b", app):
            found += 1
            ...
            if re.search(r"\b(dFdx|dFdy|fwidth)\s*\(", code) and "derivatives:true" not in call.replace(" ", ""):
                bad.append(f"{name}: uses derivatives but its material ({js_name}:{app[:start].count(chr(10)) + 1}) "
            ...
            if missing:
                bad.append(f"{name}: declares uniforms its material ({js_name}:{app[:start].count(chr(10)) + 1}) "
                           f"never provides: {', '.join(missing)}")
            else:
                print(f"{name}: {len(declared)} uniforms provided by the material at {js_name}:{app[:start].count(chr(10)) + 1}")
        if not found:
            bad.append(f"{name}: no material in {js_name} carries it (fragmentShader:{name}), so its uniforms were not cross-checked")
...
def write_shaders_js():
    ...
    # the noise lattice reader, for pages without app.js (ambient.html splices it at /*@cnoise*/)
    cn = _js_literal(open(APP).read(), "CN_NEW") if os.path.exists(APP) else ""
    assert cn and "`" not in cn and "${" not in cn, "CN_NEW"
    parts.append("CN_NEW:`%s`" % cn)
