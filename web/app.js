...
async function loadAll(){
  ...
  /* Shipped world sheets, if the bake script has run (WP-10 plan A3). Optional,
     ...
     Two sets since the port's second round (2026-09-07): sheets/ is 4096 wide
     -- what the wide view draws from on a desktop, at most 1.8x magnified
     where it engages -- and sheets2048/ is the lean set the ambient page
     runs on, which a constrained device takes instead (a 4096 sheet is
     76 MB of bitmap and mipmapped texture). */
  (async()=>{
    const dirs=((navigator.deviceMemory||8)<8)?['sheets2048/','sheets/']:['sheets/'];
    for(const dir of dirs){
      try{const r=await fetch(dir+'manifest.json?v='+SHEET_V); if(!r.ok)continue;
        const m=await r.json(); if(!(m&&m.files))continue;
        SHEET_MANIFEST=m; SHEET_DIR=dir; SHEET_KEEP=(m.w>=4096)?6:8; break;
      }catch(e){}
    }
  })();
...
window.__MEM=()=>{
  ...
  let sheetBytes=0; for(const s of SHEETS.values())if(s.tex&&s.w)sheetBytes+=s.w*s.w/2*4*(s.rt?1:(s.src?2.33:1.33));   // a shipped sheet: its mipmapped texture, plus the bitmap while its strips are still copying; a baked one: the target
  return {tried:_fieldTried.size,bitmaps:n,pinnedMB:Math.round(bytes/1048576),
          legacyImgs:legacy,cacheMB:Math.round(_texBytes/1048576),extraMB:Math.round(_extraBytes/1048576),sheetsMB:Math.round(sheetBytes/1048576),
...
function queueUploads(f){
  ...
  let decodesKicked=0;
  /* The sheet path warms every kind too, one upload a frame. Round 2 tried
     warming only the two kinds its own material reads (an upload or two of
     8-32 MB sat in most playback frames): bindTextures binds the terrain
     material every frame whatever the path, so the skipped uploads merely
     moved to the crossing frame -- eight at once, 355 ms, the storm the
     perf audit's gate exists to catch. Spread beats the storm. */
  const liteNow=globe.material===liteMat;
  for(const w of want){
    if(w<0||w>=n)continue;
  ...
  let drains=(state.playing&&!liteNow)?2:1;         // playback burns keyframes faster; the sheet path spreads them thinner
...
function initGL(){
  ...
  /* Cloud layer -- the single biggest "seen from space" cue. Since the Atlas
     ...
     starts and fades it back once the requested pair is bound -- until the
     second round, when the deck stayed through playback too (see loop). */
  ...
  const cmat=new THREE.ShaderMaterial({transparent:true,depthWrite:false,
    ...
    uniforms:{
      ...
      uSnowball:mat.uniforms.uSnowball,   // the era's ice line, shared: a snowball world loses most of its deck
  ...
  cv.addEventListener('webglcontextlost',()=>{if(_ground){_ground.rt.dispose();_ground=null;}
    for(const [i,s] of [...SHEETS])if(!s.rt)_sheetDrop(i);   // a strip-filled texture has no bitmap to re-upload from
  });
  // stars
...
const _elevPending=new Set();
function _elevStore(i,src){
  const c=document.createElement('canvas');c.width=ELEV_W;c.height=ELEV_H;
  const cx=c.getContext('2d',{willReadFrequently:true});
  drawFieldImage(cx,src,ELEV_W,ELEV_H);
...
/* The 256x128 CPU elevation raster the labels snap to. It used to be drawn
   from the keyframe's full 4096x2048 bitmap into the small canvas on the
   spot: a GPU-to-CPU read-back of 33 MB on the main thread, 150-390 ms once
   per keyframe -- the crossing hitch playback carried at every keyframe
   (round 2's storm test: a crossing with no uploads at all cost 316-387 ms,
   and one frame in twenty of sheet-path playback was that size). Now the
   raster is built OFF the main thread from a resized decode of the same
   bytes (the broker's cache or the browser's), and readers get null until
   it lands, as they always did while the elevation was in flight: a label
   rides its plate position for a moment, then snaps. The pair ahead is
   asked for at the crossing, so the raster is usually there in time. */
function elevField(i){
  if(ELEVDATA.has(i))return ELEVDATA.get(i);
  if(!_BITMAPS){const img=fieldImage('e',i);return img?_elevStore(i,img):null;}
  if(_elevPending.has(i)||_bmMissing.has('e'+i))return null;
  _elevPending.add(i);
  fieldBlob(i,'e',12)
    .then(b=>createImageBitmap(b,{imageOrientation:'flipY',premultiplyAlpha:'none',colorSpaceConversion:'none',
      resizeWidth:ELEV_W,resizeHeight:ELEV_H,resizeQuality:'high'}))
    .then(bm=>{_elevPending.delete(i);if(!ELEVDATA.has(i))_elevStore(i,bm);bm.close&&bm.close();})
    .catch(err=>{_elevPending.delete(i);if(!(err&&err.superseded))_fieldFailed('e'+i,err);});
  return null;
}
...
/* Which organisms to show for a clicked feature.
   ...
   THE CARD IS COMPOSED AT BUILD TIME (build/biota.py), not here. Every organism
   the app can show is declared once in a registry -- what it is, when it lived,
   where it lived -- and each label's card is assembled from that per age:
   the label's curated occurrences first, then its province's markers, then
   whatever else the registry places on this crust at this moment; all of it
   filtered by age and by place, and balanced between fauna and flora on
   purpose. It ships as runs over whole-Myr ages:

       cards[label] = {r: [[a_lo, a_hi, tier, provinceId, groups, shared?, curatedIds?], ...],
       ...
       tier         x  a curated EXCEPTION locality, atypical for its province
                    ...
                    r  no named province either: the registry, for this crust

   What used to live here -- three tiers with three data shapes, a realm filter
   that was a guess about the locality, a province tuple held constant for sixty
   million years -- was the source of Bison at 60 Ma and moose in Patagonia.
   build/audit_biota.py replays every one of these runs against the registry
   and fails the build on an anachronism, a misplacement, or a land card that
   shows animals and no plants or plants and no animals. */
const MARINE_T=new Set(['ocean','sea']);
function cardAt(name,age){
  const C=(DATA.life.cards||{})[name], R=C&&C.r; if(!R||!R.length)return null;
  const a=Math.round(Math.max(0,age));
  let best=null,bd=1e9;
  for(const r of R){
    if(a>=r[0]&&a<=r[1])return r;
    const d=Math.min(Math.abs(a-r[0]),Math.abs(a-r[1]));
    if(d<bd){bd=d;best=r;}
  }
  /* A label stays on the map a little past its window (labelVisible's slack),
     so its card does too; further than that and the run is about another time. */
...
function groupTitle(key,taxa,marine){
  if(key==='fauna')return 'Fauna';
  if(key==='flora')return marine?'Flora and algae':'Flora';
  if(taxa.length&&taxa.every(t=>t.kd==='deposit'))return 'In the rock';
  return marine?'Plankton and microbes':'Microbial life';
}
function lifeSection(l){
  const age=state.age, when=fmtAge(Math.round(age)), marine=MARINE_T.has(l.t);
  const run=age>-0.5?cardAt(l.n,age):null;
  if(run){
    const [,,tier,pid,groups,shared,cur]=run;
    const T=DATA.life.taxa||[], notes=((DATA.life.cards||{})[l.n]||{}).n;
    const prov=(pid!==null&&pid!==undefined)?(DATA.life.provinces||{})[pid]:null;
    const named=prov&&tier!=='x';
    let h=`<div class="ihead">${named?esc(prov.n)+' · '+when:'Life here at '+when}</div>`;
    if(named&&prov.note)h+=`<p class="desc">${esc(prov.note)}</p>`;
    /* Before the Silurian a land card has almost nothing of its own to show,
       and saying so is the interesting half of the answer. */
    /* Where the honest list is short the build says why (life.sparse), and the
       reason belongs on the card it explains -- not only where there is no card. */
    const thin=sparseAt(l.n,age); if(thin)h+=`<p class="desc">${esc(thin)}</p>`;
    const seaRun=groups.some(g=>g[0].startsWith('sea-'));
    ...
    for(const [key,ids] of groups){
      ...
      const taxa=ids.map(i=>{const t=T[i]||{}; const ln=notes&&cur&&cur.includes(i)&&notes[i];
    ...
    if(shared)h+=`<p class="desc">${esc(shared)}</p>`;
    if(named&&prov.c==='moderate'&&tier==='p')h+=`<p class="desc">The province is modelled `+
      `from palaeolatitude and the block this sat on; treat its boundary as a gradient, `+
      `not a line.</p>`;
    return h;
  }
  /* NO COMPOSED CARD. That is the projected future, a label the build could not
     give a home crust, or an age at which the registry places nothing here. The
     interval-wide list is the honest remainder, under a heading that says it is
     interval-wide -- never a claim that these lived at this spot. */
  const why=sparseAt(l.n,age);
  if(why)return `<div class="ihead">Fossil record here</div><p class="desc">${esc(why)}</p>`;
  const L=lifeAt(age);
  if(!L||!L.taxa||!L.taxa.length)return '';
  const want=marine?['sea']:(l.t==='lake'?['fresh','land','air']:['land','air','fresh']);
  const reg=l.rg||null;
  ...
  if(pick.length){
    const head=marine?`Worldwide in the sea at ${when}`:`Worldwide on land at ${when}`;
    return `<div class="ihead">${head}</div>`+lifeGroupsHTML(pick,6,marine)+
      `<p class="desc">This is the GLOBAL list for the interval — nothing is catalogued `+
      `for this spot. Not a claim that these lived here.</p>`;
  }
  if(marine)return '';
  return `<div class="ihead">On land at ${when}</div><p class="desc">`+
    (age>430
      ? `Essentially nothing. Land is bare rock, sand and microbial crust — `+
        `vascular plants do not spread until the Silurian.`
      : `No land group is catalogued for this interval yet.`)+`</p>`;
}
/* A flat list sorted into its fauna, its flora and the rest, each under its own
   sub-heading, for the lists that do not come pre-grouped (the interval cards
   and the global fallback). `k` is bound from the registry at build time. */
function lifeGroupsHTML(taxa,limit,marine){
  taxa=taxa.filter(taxonNow);
  const by={fauna:[],flora:[],other:[]};
  for(const t of taxa)(by[t.k]||by.other).push(t);
  if(!by.fauna.length||!by.flora.length)return lifeHTML(taxa,limit);
  const n=limit||99, nf=Math.min(by.fauna.length,Math.ceil(n/2)),
        nl=Math.min(by.flora.length,n-nf);
  let h=`<div class="lsub">Fauna</div>`+lifeHTML(by.fauna,Math.max(nf,n-nl-Math.min(by.other.length,1)));
  h+=`<div class="lsub">${marine?'Flora and algae':'Flora'}</div>`+lifeHTML(by.flora,nl);
  return h;
}
...
function showInterval(i){
  ...
  if(L&&L.taxa&&L.taxa.length)extra+=`<div class="ihead">Life</div>`+lifeGroupsHTML(L.taxa,8,false);
...
/* THE CORE OF A PAIR (round 2, 2026-09-07): the kinds without which the
   picture is not one world -- elevation, rainfall, lakes, surface process
   and ocean structure of both keyframes, and the interval's displacement
   and plate slots from the younger one. The first round switched from the
   preview to the terrain shader on elevation alone, and the other kinds
   landed behind it over the next second: a new coastline over the previous
   age's lakes, drainage and abyssal fabric, which reads as an unfinished
   frame. Now the preview (or a near still) stays until the core is here,
   and playback WAITS for the next pair's core the way it waits for a late
   sheet. A kind the keyframe lacks (a 404) is not waited on; the small-field
   stack, the motion field and the noise of detail still refine behind. */
function coreKinds(f){
  const out=[['e',f.i],['r',f.i],['w',f.i],['d',f.i],['o',f.i],['p',f.i]];
  if(f.j!==f.i)out.push(['e',f.j],['r',f.j],['w',f.j],['d',f.j],['o',f.j],['v',f.i]);
  return out;
}
function coreResident(f){
  for(const [k,i] of coreKinds(f)){
    const key=k+i; if(_bmMissing.has(key))continue;
    const e=TEXCACHE.get(key); if(!(e&&e.bm))return false;
  }
  return true;
}
let _fieldHolds=0;   // frames playback waited for the next pair's core (the terrain path's _liteHolds)
let _bakesStarted=0; // in-app sheet bakes begun (a shipped site should count none)
const HOLD_MS=3000;  // the longest time waits for a pair's core or a sheet; 1.5 s let a slow machine advance into a still
const MAX_STEP_MYR=1.5; // the most one frame may move time (0.3 of an interval), whatever the speed and the frame's length
/* A core that never completes (a server answering 5xx for one kind, a
   decode that keeps failing) must not leave the preview up for good: after
   CORE_PATIENCE the pair binds progressively, as it did before the round. */
const CORE_PATIENCE=6000;
let _coreWaitKey='',_coreWaitAt=0;
function coreReadyOrTired(f){
  if(coreResident(f)){_coreWaitKey='';return true;}
  const key=f.i+':'+f.j;
  if(key!==_coreWaitKey){_coreWaitKey=key;_coreWaitAt=performance.now();}
  return performance.now()-_coreWaitAt>CORE_PATIENCE;
}
...
function bindTextures(){
  ...
  const pairResident=coreReadyOrTired(f);
  /* A pair whose core is not here yet. Within STILL_RADIUS keyframes, a
     resident neighbour is the better stand-in (detailed, and at most 10 Myr
     off); farther, the preview is (exact age, coarse); with no preview
     loaded a scrub still steps through whatever is resident, as it always
     has, and a jump keeps the previous world. */
  ...
  if(!pairResident){
    // the remaining core kinds, at their essential priority, while the preview stands in
    for(const [k,i] of coreKinds(f))ensureBitmap(k,i,true);
  ...
  bindClimate(f);
  if(ea&&eb&&pairResident)_surfaceSet('full',f,[f.i,f.j]);
  return f;
}
...
let SHEET_KEEP=8;                   // sheets held: the pair, up to four ahead, the neighbours behind, a spare (6 with the 4096 set: 76 MB each)
...
function _shippedSheet(i){
  ...
  const s0=SHEETS.get(i); if(s0&&s0.src)return true;   // arriving, strip by strip
  /* A shipped sheet whose fetch or strip failed is tried again in a while,
     and until then it is still "on its way": returning false here made
     pumpBakes bake it in the app meanwhile -- four strips of an 8-megapixel
     terrain render a frame, 1.8 s frames in the round-2 playback test. A
     site that ships its sheets never bakes one. */
  if((_sheetRetryAt.get(i)||0)>performance.now())return true;
  _shippedPending.add(i);
  LOADER.get((SHEET_BASE||SHEET_DIR)+file+'?v='+SHEET_V,25,i)
    .then(b=>createImageBitmap(b,{imageOrientation:'flipY',premultiplyAlpha:'none',colorSpaceConversion:'none'}))
    .then(bm=>{
      /* THE STRIP UPLOAD (round 2, 2026-09-07). A 4096 sheet is 32 MB of
         pixels and a mip chain; uploaded whole at its first draw it cost the
         crossing frame ~70 ms (the storm gate: 78 ms against 6 with the
         2048 set). So the texture is allocated empty now and filled in
         sixteen strips over the following frames -- the same way the bakes
         render and the small-field stack fills -- with the mip chain built
         once on the last strip; the sheet reports ready only then, and
         playback waits for it as for any late sheet. The decoded bitmap is
         closed once copied, so a shipped sheet resident costs its texture
         alone (44 MB) instead of texture plus bitmap. */
      const t=new THREE.DataTexture(null,bm.width,bm.height,THREE.RGBAFormat,THREE.UnsignedByteType);
      t.flipY=false; t.premultiplyAlpha=false; t.colorSpace=THREE.NoColorSpace;
      t.minFilter=THREE.LinearMipmapLinearFilter; t.magFilter=THREE.LinearFilter; t.generateMipmaps=true;
      t.wrapS=THREE.RepeatWrapping; t.wrapT=THREE.ClampToEdgeWrapping;
      t.anisotropy=Math.min(4,renderer.capabilities.getMaxAnisotropy()); t.needsUpdate=true;
      renderer.initTexture(t);      // the storage, allocated in this frame; the pixels follow a strip at a time
      t.generateMipmaps=false;      // the last strip turns it back on for one generateMipmap
      _sheetDrop(i); SHEETS.set(i,{rt:null,tex:t,w:bm.width,ready:false,u:++_sheetClock,src:bm,launched:0,done:0,inflight:0,strips:16});
...
/* Strips of one arriving shipped sheet: crop each off the decoded bitmap
   (off the main thread), copy it into the allocated texture. HOW MANY A
   FRAME FOLLOWS THE FRAME: one strip per 16 ms of the last interval, so a
   60 fps loop pays a strip a frame and a 300 ms terrain frame pays eight
   (a few ms more on a frame that long) -- without that, a wide view on a
   slow GPU filled its sheets at three frames a second, five seconds a
   sheet, and stayed on the terrain shader that made the frames slow in
   the first place. Strips complete in any order; the mip chain is built
   with the last copy. */
function pumpSheetStrips(rawMs,urgent){
  /* Strips a frame. Two while the picture has its sheets (a 4096 strip is
     2 MB of texSubImage2D; sixteen land in eight frames): the first rule,
     round(rawMs/16), fed on itself -- a slow frame allowed five strips,
     which kept it slow, 83 ms at 3 Myr/s on the sheet path. Up to eight,
     by the frame's length, while the picture is WAITING for a sheet: the
     terrain path at 350 ms a frame needs the sheet in two frames, and one
     strip a frame there was five seconds of crawl. */
  const per=urgent?Math.max(2,Math.min(8,Math.round((rawMs||16)/16))):2;
  for(const [i,s] of SHEETS){
    if(!s.src)continue;
    while(s.inflight<per&&s.launched<s.strips){
      const row=s.launched++, H=s.src.height, y0=Math.floor(row*H/s.strips), y1=Math.floor((row+1)*H/s.strips);
      s.inflight++;
      createImageBitmap(s.src,0,y0,s.src.width,y1-y0,{premultiplyAlpha:'none',colorSpaceConversion:'none'})
        .then(strip=>{
          s.inflight--;
          if(SHEETS.get(i)!==s){strip.close();return;}          // dropped while the strip was cut
          const last=(s.done===s.strips-1);
          s.tex.generateMipmaps=last;
          const gl=renderer.getContext(); gl.pixelStorei(gl.UNPACK_COLORSPACE_CONVERSION_WEBGL,gl.NONE);
          renderer.copyTextureToTexture(_stkPos.set(0,y0),{image:strip},s.tex);
          strip.close(); s.done++;
          if(last){s.src.close();s.src=null;s.ready=true;s.u=++_sheetClock;}
        })
        .catch(()=>{s.inflight--;if(SHEETS.get(i)===s){_sheetDrop(i);_sheetRetryAt.set(i,performance.now()+3000);}});
    }
...
function pumpBakes(f,rawMs){
  if(liteMode==='off'||!DATA.timeline||!renderer)return;
  /* Waiting for a sheet: the pair's own is not ready, or playback reaches
     the next keyframe within a second and its sheet is not ready either. */
  let urgent=!(sheetReady(f.i)&&sheetReady(f.j));
  if(!urgent&&state.playing){
    const nx=state.dir<0?f.i-1:f.j+1, A=ages();
    if(nx>=0&&nx<A.length&&!sheetReady(nx)){
      const edge=state.dir<0?A[f.i]:A[f.j];
      urgent=Math.abs(state.age-edge)/Math.max(0.1,state.speed)<1.0;
    }
  }
  pumpSheetStrips(rawMs,urgent);
  ...
  for(const i of want){
    ...
    _bakeJob={i:i,rt:s.rt,row:0}; _bakesStarted++; _bakeStrips(); return;
  }
}
...
function useLite(f){
  ...
  /* EAGER ON A SLOW GPU (round 2, 2026-09-07). Once the governor has had to
     step the render scale down -- or the quality is pinned below Full --
     the sheets engage from a quarter of a texel per pixel instead of half:
     a pre-shaded sheet magnified up to four times is sharper than the live
     shader at half resolution, and a tenth of its cost, which is what makes
     wide-zoom playback smooth on the M1's 5K display. At full scale the
     old half-texel rule stands. */
  const eager=(state.quality==='auto'&&_autoScale!=='full')||state.quality==='balanced'||state.quality==='perf';
  const on=eager?0.25:0.45, off=on*0.78;
  _liteOn = _liteOn ? r>off : r>on;
  return _liteOn;
}
...
function loop(now,force){
  ...
  if(DATA.timeline){const f0=curFrame();retargetLoads(f0);pumpBakes(f0,rawMs);}
  if(state.playing){
    /* THE LONGEST STEP (the port's second round). dtT keeps the slider's
       Myr/s honest against the wall clock, but at 10 Myr/s a 300 ms frame
       moved time 3 Myr -- more than half an interval, a continent leaping
       14 texels -- which is what "the continents jump" looked like. No frame
       moves time by more than MAX_STEP_MYR: at the default 3 Myr/s the half-
       second cap already held it there, so only fast playback on slow frames
       slows below the slider, and smoothly. */
    const step=state.dir*Math.min(state.speed*dtT,MAX_STEP_MYR);
    ...
    if(globe.material===liteMat&&!(sheetReady(nf.i)&&sheetReady(nf.j))){
      if(!loop._holdAt)loop._holdAt=now;
      hold=(now-loop._holdAt)<HOLD_MS;
      if(hold)_liteHolds++;
    }
    if(!hold&&!coreResident(nf)){
      /* TIME WAITS FOR THE FIELDS TOO (round 2, 2026-09-07), on both paths.
         Time used to advance into a pair whose fields were still arriving,
         and the picture snapped -- to a still of the last keyframe, or to
         the previous pair under a new age -- and snapped again when the
         fields landed: continents that jump instead of drifting, and on the
         sheet path a coastline mask and a warp from the wrong keyframe (at
         10 Myr/s the first cut spent four frames in five on a still, and
         the clouds flickered with them). Time now waits for the next pair's
         core, capped at HOLD_MS so a field that never comes cannot stop the
         clock; the decodes are asked for at essential priority meanwhile. */
      for(const [k,i] of coreKinds(nf))ensureBitmap(k,i,true);
      if(!loop._holdAt)loop._holdAt=now;
      hold=(now-loop._holdAt)<HOLD_MS;
      if(hold)_fieldHolds++;
    }
  ...
  /* CLOUDS (the Atlas port, round 2). Wanted whenever the layer is on and
     the requested pair is bound at full detail -- playing or paused: since
     the second round the deck rides the running timeline, its transport on
     the weather clock and its arrangement following the era's land, rain
     and ice, and playback WAITS for a pair's core (above) so the clouds
     never see a half-arrived world. They fade over 1.4 s on the switch and
...
(async function(){
  ...
  window.APP={state,DATA,mat,TEXCACHE,snapLabel,projectLL,curFrame,elevField,elevAtLL,   // mat/TEXCACHE: the harness reads uniforms and residency
              ...
              sheets:{
                status(){return {mode:liteMode,w:SHEET_W,h:SHEET_H,lite:globe.material===liteMat,frames:_liteFrames,holds:_liteHolds,fieldHolds:_fieldHolds,bakes:_bakesStarted,held:SHEETS.size,keep:SHEET_KEEP,dir:SHEET_DIR,shippedW:SHEET_MANIFEST&&SHEET_MANIFEST.w,
