...
uniform vec4 uEroK3;     // ...and their LIGHT: x shading ratio per sub-grid octave below 12 km (?eroG=), y how hard their walls steer the next (?eroW=), z their pivots' spacing and heading jitter (?eroJ=), w wavelengths per pivot cell (?eroC=)
uniform float uFloorK;    // pale arid basin floors (?floor=0 off)
uniform float uPeakK;     // sub-grid peaks in the snow and glacier ramps (?peak=0 off)
uniform float uMeeK;      // the mass-elevation effect (?mee=0 off)
...
float gIceA=0.0;    // the land-ice opacity the ice block painted (0 bare ground .. 1 a solid sheet)
float gSnowC=0.0;   // the snow and glacier cover the land block painted (for the steep-rock term)
...
vec3 eroPlane(vec2 p, vec2 n, float jit, float cpc){
  ...
  for(int j=-1;j<=1;j++){
    for(int i=-1;i<=1;i++){
      ...
      float ja=(eroHash(ip+o+vec2(5.3,71.9)).x-0.5)*0.87*jit;
      vec2 nj=n*cos(ja)+vec2(-n.y,n.x)*sin(ja);
      /* (below the grid, where the pivots barely jitter, the SPACING varies
         by region instead: blocks of 4x4 cells share one, 0.8-1.25x, so a
         uniform slope is not combed at one pitch from end to end -- dissection
         density follows the rock, and the rock changes every hundred km) */
      float fr=mix(0.8+0.45*eroHash(floor((ip+o)*0.25)+vec2(91.7,13.3)).x, 1.0, jit);
      float ph=6.2831853*cpc*fr*mix(1.04, 0.78+0.52*hq.y, jit)*dot(d,nj);
...
vec3 eroOct(vec3 m, vec3 n3, vec3 aw, float jit, float cpc){
  vec3 a=vec3(0.0);
  if(aw.x>0.02) a+=aw.x*eroPlane(m.yz, normalize(n3.yz+vec2(1e-7,0.0)), jit, cpc);
  if(aw.y>0.02) a+=aw.y*eroPlane(m.zx, normalize(n3.zx+vec2(1e-7,0.0)), jit, cpc);
  if(aw.z>0.02) a+=aw.z*eroPlane(m.xy, normalize(n3.xy+vec2(1e-7,0.0)), jit, cpc);
  float l=length(a.xy);
...
vec4 eroRelief(vec3 msd, vec3 Gw, vec3 Aw, float aC, float aF, float pxKm){
  ...
  for(int k=0;k<8;k++){
    ...
    if(max(a,aS)*fade>0.5){
      ...
      vec3 n=cross(msd,t);
      /* BELOW THE GRID A GULLY RUNS THE LENGTH OF ITS WALL. The spacing and
         heading jitter that keeps the 96 and 48 km octaves from reading as a
         grating at globe zoom broke a 12 km gully after about one wavelength
         at close zoom -- the dashes of the herringbone. Real first-order
         valleys run crest to floor, many wavelengths, and bend with the
         slope that steers them; so below 12 km the jitter is uEroK3.z of it.
         The jitter was not what broke them, though (measured: ?eroJ=0.35 left
         the dashes as they were). Each pivot draws its stripe through itself
         and the strongest pivot changes every cell, so at one cycle a cell
         the phase jumps once a wavelength along any gully. Below 12 km the
         cells are uEroK3.w wavelengths across, at as many cycles each: the
         same nine pivots a pixel, and gullies that many wavelengths long. */
      float jit=k>=3 ? uEroK3.z : 1.0;
      float cpc=k>=3 ? uEroK3.w : 1.0;
      vec3 e=eroOct(msd*(6371.0/(L*cpc))+vec3(float(k)*7.31,float(k)*3.17,float(k)*5.53), n, aw, jit, cpc);
      ...
      /* THE SUB-GRID OCTAVES ARE LIT ON A FALLING SPECTRUM (the herringbone
         round, 3.16). Their HEIGHT keeps 0.78 per octave -- the snowline, the
         bare rock and the treeline need the relief -- but lit at the same
         ratio each finer octave's walls are 1.56x steeper, and with the gain
         falling only as sqrt(L) the finest octave drawn out-shaded all the
         rest: short hard dashes, each run straight down the wall of the one
         above -- a herringbone on every Himalayan flank at close zoom
         (?show=13). Real shaded relief is led by its main valleys, the finer
         dissection a texture on their walls: below 12 km each octave is lit
         at uEroK3.x of the one above. */
      float shK=k>3 ? pow(uEroK3.x, float(k-3)) : 1.0;
      gr+=(ae*dtri*kk*sqrt(L*(1.0/96.0))*shK)*n;
      ...
      /* (Below 12 km the walls steer at uEroK3.y instead: at 1.8 a tributary
         ran straight down its valley's wall and met it square -- the other
         half of the herringbone. With the flank's own fall counting for more,
         it slants downstream and joins at the acute angle real drainage does.) */
      gi+=((k>=3 ? uEroK3.y : 1.8)*aS*mask*kk*dtri)*n;
...
void main(){
  ...
  /* ...EXCEPT THE LIGHT (3.16). pwL turns the sun from the north-west of the
     local frame into the world-fixed ring frame, and it began at 51 degrees
     with the sampling correction: across 51-80 the light's direction swung
     with longitude, so whole coasts were lit from behind -- east Baffin, lit
     from the east-south-east, drew as a flat dark band 300 km long (?show=15),
     the "dark bare-rock areas on present-day polar land". The light needs the
     ring frame only where north-west stops meaning anything, at the pole
     itself, so it turns over 72-88 degrees; the sampling keeps its wide ramp. */
  float pwL=smoothstep(0.9511,0.9994,abs(sdir.y));
  ...
  vec3 gEnv3=vec3(0.0); float gMac=0.0; float gMacMin=0.0;
  ...
  if(z<wl){
    ...
    gMacMin=min(min(mE1,mE2),min(mN1,mN2));   // the massif's interior (the mass-elevation effect)
  ...
  /* THE COMPRESSION RUNS IN BOTH LATITUDE BRANCHES (3.16). It sat inside the
     sub-63 condition with the fine band, which needs the +-1-texel taps that
     branch has; the compression needs nothing but the gradient. Above 63
     degrees a broad flank facing away from the polar frame's light was lit at
     the full regional tilt and drew as a dark slab -- east Baffin, the
     "dark bare-rock areas on present-day polar land". */
  if(z>=wl && uHsK.x>0.0){
    vec2 gb=vec2(gE,gN);
  ...
  float zp=max(z,0.0);
  /* THE MASS-ELEVATION EFFECT (3.16). The lapse rate above is the free air's;
     a high plateau heats its own air, so its surface stands several degrees
     warmer than the free atmosphere at the same height -- measured ~4-6 C over
     interior Tibet, ~1-2 C in the Alps -- which is why the Tibetan treeline and
     snowline are the highest on Earth. Without it the plateau was too cold,
     snow lay across its interior while the Himalayan crest stood bare, and the
     Alps glaciated from ~2,700 m against a real ~2,900. Keyed to the mean of the
     four +-137 km taps (the massif, not the peak) and, weighted three to one,
     their minimum (how far inside it this point is): a plateau interior, high
     on every side, takes ~4.5 C; a crest with a plain on one side, under 1 C
     -- measured by the mean alone, the Himalayan crest took 3.8 C and lost all
     its snow, and the south-side ELA there is ~5,600 m, which the lapse rate
     alone already gives.
     1.1 C per km above 500 m, capped at 5.5; faded out in polar climates,
     where an ice sheet cools its air rather than heating it. It moves the
     snowline, the glacier line and the treeline with the temperature.
     ?mee=0 removes it. */
  float T0sl=(26.0-24.0*s2-26.0*s2*s2*s2)+(uTemp-TEMP_REF)*(4.0+15.0*s2);
  float mee = z>=wl ? uMeeK*min((0.25*max(gMac-500.0,0.0)+0.75*max(gMacMin-500.0,0.0))*0.0011, 5.5)
                      *clamp((T0sl+5.0)/15.0,0.0,1.0) : 0.0;
  float T=T0sl+mee-zp*0.0058;
  ...
  if(z<wl){
    ...
    /* Mountain glaciers from an equilibrium-line altitude, not a bare snowline.
       ...
       whole shader.) */
    float T0=T0sl+mee;                     // the mass-elevation effect raises every line keyed to T0
    /* THE MOISTURE THAT REACHES A CREST (3.16). Orographic rain peaks on the
       windward slope below a crest, and a 26 km rain grid books it there; the
       crest itself still takes much of it as snow -- the monsoon feeds the
       Himalayan crest's glaciers and the rain shadow begins BEHIND it. Read at
       its own texel the crest sat on the dry side of that step (0.05 against
       0.4 on the slope below) and the continuous snow line the Himalaya shows
       from orbit was a scatter of specks. On high ground the glacier line and
       the snowfall take the wettest rain within ~50 km at 0.8 weight; the
       treeline and the vegetation keep their own texel's. */
    float RfE=Rf;
    if(zp>1800.0){
      float cl=sqrt(max(1.0-s2,0.04));
      vec2 oE=vec2(0.45/360.0/cl, 0.0), oN=vec2(0.0, 0.45/180.0);
      float r1=mix(texture2D(rainA,wA(uv+oE)).r, texture2D(rainB,wB(uv+oE)).r, mixf);
      float r2=mix(texture2D(rainA,wA(uv-oE)).r, texture2D(rainB,wB(uv-oE)).r, mixf);
      float r3=mix(texture2D(rainA,wA(uv+oN)).r, texture2D(rainB,wB(uv+oN)).r, mixf);
      float r4=mix(texture2D(rainA,wA(uv-oN)).r, texture2D(rainB,wB(uv-oN)).r, mixf);
      RfE=max(Rf, 0.8*RF_MAX*max(max(r1,r2),max(r3,r4))*smoothstep(1800.0,2600.0,zp));
    }
    float arid=1.0-clamp(RfE/0.85,0.0,1.0);
    ...
    /* PEAKS THE GRID CANNOT SEE (3.16). A 10 km cell of the Himalayan crest
       reads 5,000-5,800 m while its peaks stand at 7,000-8,800: the cell is
       partly above the glacier line even when its mean is not. So on rugged
       high ground the ramp widens by the unresolved relief (sigS, a standard
       deviation, from rug), about the same centre -- rugged ranges carry ice
       and snow on their peaks, a smooth plateau at the same mean height does
       not, which is the pattern from orbit. The spread is sized by how far the
       point stands above its massif (zp - gMac) and by its ruggedness: ~1 km on
       the Himalayan crest, where 10 km cells run from 4.5 km valleys to 7.5 km
       peaks, ~0.8 in the Karakoram, under 0.2 on the plateau interior.
       ?peak=0 restores the plain ramp. */
    float sigS=uPeakK*min(0.55*max(zp-gMac,0.0)+300.0*rug, 1000.0)*smoothstep(1500.0,3200.0,zp);
    float glac=clamp((zp-ela+0.8*sigS)/(520.0+1.6*sigS)+tongue+0.08,0.0,1.0);
    ...
    /* THE RAMP IN MILLIMETRES (3.16). On the model's scale -- which the rain
       anchor's quantile mapping pins to observation -- 0.04 is ~300 mm and
       0.42 ~1,500, so a range needed 1.5 m of precipitation before its snow
       showed in full; the ELA above already charges dryness. 0.015-0.22 is
       ~130 mm to ~1,000: the Atacama's volcanoes (<50 mm) stay at the floor,
       a 500 mm crest shows most of its snow. */
    float snowfall=clamp(0.30+0.70*smoothstep(0.015,0.22,RfE),0.0,1.0);
    ...
    snowfall=mix(snowfall,1.0,clamp((zp+sigS-snowline)/1100.0,0.0,1.0)*0.80);   // the peaks' excess, sigS below
    ...
    float snow=clamp((zp-snowline+sigS)/(400.0+2.0*sigS)+tongue*0.6,0.0,1.0)*snowfall;
    gDbgV=vec4(snow, sigS/1000.0, mee/6.0, arid);
    snow=max(snow,glac);
  ...
  /* A range that breaks through the sheet (the bed 0.8-1.6 km above the four
     +-137 km taps: the Transantarctic Mountains, not the buried Gamburtsevs)
     keeps its relief under every ice term below and its rock above (3.16). */
  float nunIce=smoothstep(800.0,1600.0,zp-gMac);
  float eroIce=1.0-landIce*mix(0.85,1.0,gIceDeep)*(1.0-nunIce);
  ...
  /* ...AND AN ICE DOME IS NEARLY FLAT (3.16). Keeping the regional slope at
     the terrain's exaggeration lit a sheet's gentle dome like a mountain
     flank: Greenland's surface -- the DEM carries it, 0.3-1 degree slopes --
     drew with a grey half 1,000 km across, and Antarctica's bed (the DEM
     carries the bed there) with grey swathes where no ice surface slopes that
     way. The macro term carries the regional slope at ~3x the local one's
     weight, so Greenland's 0.33-degree dome became a 36-degree tilt even at a
     fifth (?show=15: its west half black). Deep inside a sheet the shading
     keeps 5% of the regional slope -- a dome reads as a gentle gradient of
     light, ~10 degrees at most -- and nothing finer; nunataks keep theirs. */
  if(z>=wl && landIce>0.001){
    float fIce=landIce*gIceDeep*(1.0-nunIce)*0.95;
    vec2 gSm=gMacD*(2.4/14.0+0.5)*mix(1.0,0.05,gIceDeep);
  ...
  if(z>=wl){
    ...
    /* ...BUT NOT THROUGH AN ICE SHEET (3.16). This runs after the ice, and the
       present-day DEM gives Antarctica as BEDROCK: every steep face of the
       buried subglacial ranges was painted rock over the sheet -- grey patches
       across the whole interior, the "dark bare-rock areas on present-day
       polar land", which July's renders did not have. What breaks through a
       real sheet is a range that stands well above its surroundings (the
       Transantarctic Mountains; the Gamburtsevs, 1 km above theirs, are
       buried), so under ice the bed must stand 0.8-1.6 km above the four
       +-137 km taps. The valley and atlas tones below take the same gate. */
    float thruIce=1.0-gIceA*(1.0-nunIce);
    bare*=thruIce;
    /* ...and not over a snowfield or a glacier (3.16). This runs after the
       snow, so every steep face above the snowline was repainted rock at up
       to 60% -- the Himalayan crest, snow-covered in the model, drew grey.
       Only the steepest faces shed their snow; on a glaciated crest most of
       the surface is white. */
    bare*=1.0-0.75*gSnowC;
    /* A nunatak is rock by its STANDING, not its lit slope: the regional-tilt
       compression that keeps a flank from clamping black also takes a
       2 km range front to ~34 degrees, under this slope ramp, and the
       Transantarctic Mountains -- dark rock ridges with white outlet glaciers
       across them, from orbit -- went white. Where the bed breaks through a
       sheet, broken or steep ground is rock. */
    bare=max(bare, nunIce*gIceA*max(smoothstep(0.30,0.70,rugC), smoothstep(0.05,0.18,slope)));
    ...
    if(gEroOwn>0.003){
      ...
      col*=1.0+et*0.13*gEroOwn*thruIce;
    }
    ...
    if(uAtlasOn>0.5){
      ...
      if(ga>0.005){
        ...
        col*=1.0+ar.x*0.55*ga*uAtlasK.y*eB*thruIce;
      }
      ...
      if(gd>0.005){
        ...
        col*=1.0+dr.x*0.32*gd*uAtlasK.w*thruIce;
      }
    }
    ...
    /* Under an ice sheet none of this is the surface (3.16): the grain, the
       drainage carved into the bed, the atlas, dissection and dune relief all
       belong to ground the ice covers; nunataks keep theirs. */
    nrm=normalize(nrm+vec3(nx,ny,0.0)*(0.36+0.50*rug)*(1.0-0.70*flatq)*(1.0+0.55*closeG)*thruIce);
  } else {
  ...
  if(uShow>0.5 && z>=wl){
    ...
    if(uShow>13.5) col = uShow<14.5 ? gAlb : uShow<15.5 ? vec3(clamp(gShd*0.62,0.0,1.0))
                     : uShow<16.5 ? vec3(vnoise3(matDir(sdir)*30.0), vnoise3(matDir(sdir)*30.0+7.0), 0.5)
                     : vec3(clamp(uShow<17.5 ? gDbgV.x : uShow<18.5 ? gDbgV.y : uShow<19.5 ? gDbgV.z : gDbgV.w, 0.0, 1.0));
  }
